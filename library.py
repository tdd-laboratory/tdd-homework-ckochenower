...
_date_dd_mmm_yyyy_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')

def dates_dd_mmm_yyyy(text):
    '''Find DD MMM YYYY dates in text'''
    for match in _date_dd_mmm_yyyy_pat.finditer(text):
        yield('date', match)

def dates_iso8601(text):
    '''Find iso8601 (YYYY-MM-DD) dates in text'''
